...
impl Dispatcher {
    pub(crate) fn spawn(
        ...
        *this.handle.lock().unwrap() = Some(thread::spawn(move || {
            loop {
                ...
                match dispatcher.worker_channel.sender.try_send(signal) {
                    Ok(_) => {
                        println!(
                    ...
                    Err(TrySendError::Full(signal)) => {
                        ...
                        dispatcher.waiting_queue.lock().unwrap().push_back(signal);
                    }
            ...
            if dispatcher.is_waiting.load(Ordering::Relaxed) {
    ...
    fn process_waiting_queue(&self, task_receiver: &mpsc::Receiver<Signal>) -> bool {
        ...
        match task_receiver.try_recv() {
            Ok(signal) => {
                println!("[dispatcher][proc_wait_que] got signal, add to wait_que'");
